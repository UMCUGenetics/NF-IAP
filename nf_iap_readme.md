...
**NF-IAP analysis (original IAP version)**  
WGS reads were processed by following the Genome Analysis ToolKit (GATK, v4.1.3.0) best practices guidelines where FASTQC (v0.11.5) was applied for quality control of the raw data. Sequencing reads were mapped onto the human genome (GRCh37/hg19) using Burrows Wheeler Aligner (v0.7.17). As part of GATK best practices, duplicate reads were marked with Sambamba (v0.6.8), VariantRecalibrator was used for recalibration of base quality scores, and single nucleotide variants (SNVs) and insertions and deletions (indels) were called using HaplotypeCaller (GATK v4.1.3.0). Next, copy number variants (CNVs) were called using Control-FreeC (v11.5) and structural variants were called using Manta (v1.6.0). 